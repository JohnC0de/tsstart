...
import { listPostQuery, useCreatePostMutation, useDeletePostMutation } from '@/hooks/post'
import { CreatePostSchema, PostType } from '@/lib/db/schema/post'
      ...
      <main className="flex-1">
        <section id="open-source" className="container py-8 md:py-12 lg:py-12">
          <div className="mx-auto flex max-w-[58rem] flex-col items-center justify-center gap-4">
            ...
            <a href="https://github.com/ally-ahmed/tss-app" target="_blank" rel="noreferrer" className="flex pb-2">
...
function Auth() {
  const [isRedirectingToGithub, setIsRedirectingToGithub] = React.useState(false)
    ...
    <Card>
      ...
      <CardFooter className="flex justify-between">
        ...
        {currentUser?.id === user.id && (
          <Button variant="outline" size="icon" disabled={deletePostMutation.isPending} onClick={deletePost}>
    ...
    <div className="flex flex-col gap-y-4 w-full max-w-3xl">
      {posts.length > 0 ? (
        posts.map((post, index) => (
          <Post key={index} title={post.title} body={post.body} id={post.id} user={post.user} />
        ))
      ) : (
        <h3 className="pt-8 text-muted-foreground text-2xl text-center font-semibold">No posts yet</h3>
      )}
    </div>
  )
}
    ...
    <Card className="w-full max-w-3xl">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className=" pt-6">
          ...
          <CardFooter>
            <Button className="w-full" type="submit" disabled={createPost.isPending || !user}>
