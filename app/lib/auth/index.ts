...
import type { SessionType, UserType } from '@/lib/db/schema/auth'
...
const adapter: DatabaseAdapter<SessionType, UserType> = {
  getSessionAndUser: async (sessionId: string): Promise<SessionAndUser<SessionType, UserType>> => {
    const result =
      (await db
        .select({
          user: User,
...
export const auth = cachedFunction(async () => {
  ...
  if (result.session !== null && Date.now() >= result.session.expiresAt.getTime()) {
    const session = createSession(result.user.id)
    const sessionCookie = lucia.createSessionCookie(result.session.id, session.expiresAt)
