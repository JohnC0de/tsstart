import { CreatePostSchema, Post } from '@/lib/db/schema/post'
...
export const remove = createServerFn(
  'POST',
  protectedProcedure.input(z.string()).mutation(async ({ input: postId, ctx }) => {
    console.log(`Deleting post with id ${postId}...`)
    const post = await ctx.db.query.Post.findFirst({
      where: (fields, { eq }) => and(eq(fields.id, postId), eq(fields.userId, ctx.auth.user.id)),
    })
    if (!post) throw new TRPCError({ code: 'NOT_FOUND' })
    return await ctx.db
      .delete(Post)
...
export const create = createServerFn(
  'POST',
  protectedProcedure.input(CreatePostSchema).mutation(async ({ input, ctx }) => {
